...
// Entrance/exit handled by the parent grid's stagger via variants.
const variants = {
...
/**
 * A single room tile.
 *  - Pointer-driven 3D tilt (useMotionValue → useTransform → useSpring)
 *  - Hover lift (scale) + status-colored glow
 *  - `layoutId` so the tile transitions smoothly when the grid re-orders/filters
 *
 * Tilt lives on an inner layer so it never fights Framer's `layout` transform
 * on the outer element. All motion is disabled under prefers-reduced-motion.
 *
 * forwardRef: as a direct child of <AnimatePresence mode="popLayout">, Framer
 * attaches a ref to measure the element during exit — so we pass it through.
 */
const RoomCard = forwardRef(function RoomCard({ room, onClick }, ref) {
  ...
  return (
    <motion.div
      ...
      onMouseLeave={reset}
      onClick={onClick}
      className="group relative cursor-pointer [perspective:900px]"
    >
      <motion.div
      ...
      >
        ...
        {/* top status accent */}
        <div className={`absolute inset-x-0 top-0 h-1 ${cfg.bar} opacity-80`} />
        ...
        <div className="relative mt-2 flex items-center gap-1.5 text-sm text-slate-300">
